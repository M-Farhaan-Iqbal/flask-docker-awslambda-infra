...
resource "aws_db_instance" "postgres_db_prod" {
...
output "db_instance_endpoint" {
  value       = aws_db_instance.postgres_db_prod.endpoint
  description = "The endpoint URL of the RDS instance"
}

output "db_instance_username" {
  value       = aws_db_instance.postgres_db_prod.username
  description = "The username of the RDS instance"
}

output "db_instance_name" {
  value       = aws_db_instance.postgres_db_prod.name
  description = "The name of the RDS database"
}

output "db_instance_port" {
  value       = aws_db_instance.postgres_db_prod.port
  description = "The port on which the RDS instance is listening"
}


# Create AWS Lambda function using the Docker image from ECR
resource "aws_lambda_function" "my_lambda_function_prod" {
  ...
  environment {
    variables = {
      PGHOST     = aws_db_instance.postgres_db_prod.address
      PGPORT     = aws_db_instance.postgres_db_prod.port
      PGDATABASE = aws_db_instance.postgres_db_prod.name
  ...
  depends_on = [aws_cloudwatch_log_group.lambda_log_group_prod]
}
