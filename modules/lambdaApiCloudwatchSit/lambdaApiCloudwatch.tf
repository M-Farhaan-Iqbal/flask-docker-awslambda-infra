...
resource "aws_cloudwatch_log_group" "lambda_log_group_sit" {
  name = "sitcloudwatch-log-group"
}

resource "aws_iam_role" "lambda_role" {
  name = "lambda-role-sit"
...
resource "aws_db_instance" "postgres_db_sit" {
...
resource "aws_lambda_function" "my_lambda_function_sit" {
  ...
  environment {
    variables = {
      PGHOST     = aws_db_instance.postgres_db_sit.address
      PGPORT     = aws_db_instance.postgres_db_sit.port
      PGDATABASE = aws_db_instance.postgres_db_sit.name
  ...
  depends_on = [aws_cloudwatch_log_group.lambda_log_group_sit]
}
